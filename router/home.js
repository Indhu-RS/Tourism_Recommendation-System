...
//   const { id,name, location, description, opening_hours, admission_fee ,image,district_id} = req.body;

//   db.query(
//     'INSERT INTO tourism_database.tourist_spots (spot_id,place_name, location, description, opening_hour, entry_fee,IMAGE,district_id) VALUES (?,?, ?, ?, ?, ?,?,?)',
//     [id,name, location, description, opening_hours, admission_fee,image,district_id],
//     (err, results) => {
//       if (err) {
...
//     [ id,name, rating, street, city, pincode ,image],
//     (err, results) => {
//       if (err) {
//         console.error('Error inserting tourist spot:', err);
//         res.status(500).send('Internal Server Error');
...
//   const { id, name, rating, street, city, pincode, image, district_id, number } = req.body;

//   db.query(
//     'INSERT INTO tourism_database.hotels (id, name, rating, street, city, pincode, image_url, district_id, number) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
//     [id, name, rating, street, city, pincode, image, district_id, number],
//     (err, results) => {
//       if (err) {
...
//   db.query(
//     'UPDATE tourism_database.hotels SET name=?, rating=?, street=?, city=?, pincode=?, image_url=?, district_id=?, number=? WHERE id=?',
//     [name, rating, street, city, pincode, image, district_id, number, id],
//     (err, results) => {
//       if (err) {
...
//       });
//     });
// Router.get('/complete_view', (req, res) => {
//       const query = `
//       SELECT DISTINCT districts.district_id, districts.district_name, restaurants.name, tourist_spots.Place_name
//       FROM districts
//       JOIN restaurants ON districts.district_id = restaurants.district_id
//       JOIN tourist_spots ON districts.district_id = tourist_spots.district_id;
...
//         // Render the EJS file with the joined data
//         res.render('complete_view', { data: results });
//       });
//     });
// module.exports = Router;
...
Router.post('/mysore_price', (req, res) => {
  ...
  db.query(
    ...
    [hotelId, numberOfBeds, numberOfDays],
    (err, results) => {
      if (err) {
